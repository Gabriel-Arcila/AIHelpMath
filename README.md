...
- Sub-módulos dentro de un dominio: Cuando un dominio agrupa múltiples entidades con lógica CRUD independiente, se permite crear sub-módulos anidados (ej. `src/users/users/`, `src/users/user_level/`). Cada sub-módulo contiene su propio `router.py`, `service.py`, `dependencies.py` y `repository.py`. Los archivos compartidos del dominio (`models.py` y `schemas.py`) permanecen en la raíz del módulo padre.
